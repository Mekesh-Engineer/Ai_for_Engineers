...
class ProjectAnalyzer:
    """Scans and analyzes local workspace repositories, dependencies, architecture, and code health."""
    ...
    def perform_heuristic_checks(self) -> List[Dict[str, Any]]:
        """Perform static rule-based sanity checks on project health."""
        checks = []
    ...
    async def analyze_project(
        ...
        """Generate comprehensive LLM-driven architectural and quality analysis."""
        ...
        prompt = (
            ...
            "1. **Project Overview & Architecture**: Core objective, design patterns, and module separation.\n"
            "2. **Key Components & Flow**: Important files, APIs, and data pipelines.\n"
            "3. **Dependencies & Environment**: Frameworks, model runtimes, and compatibility.\n"
            "4. **Potential Problems & Code Smells**: Bottlenecks, missing validations, or risks.\n"
            "5. **Strategic Recommendations**: Concrete next steps for enhancement and reliability."
        )

        res = await provider.generate(
            prompt=prompt,
            system_prompt=settings.get_profile_prompt("project_analyst"),
            temperature=0.3
        )
    ...
    async def debug_issue(
        ...
        res = await provider.generate(
            prompt=prompt,
            system_prompt=settings.get_profile_prompt("debugging"),
            temperature=0.2
        )
