...
class TestLocalLLMGrammarStudio(unittest.TestCase):
...
The datas collected by sensors was showing many error.
Neural network are very fast but it require GPU for speed up.
"""
        doc = file_service.extract_text_from_bytes("sample_errors.txt", sample_text.encode("utf-8"))
    ...
    def test_07_file_upload_and_proofread_api(self):
        ...
        res = self.client.post(
            "/api/files/upload",
            files={"file": ("test_doc.txt", sample_content, "text/plain")}
        )
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertEqual(data["status"], "success")
        self.assertEqual(data["document"]["filename"], "test_doc.txt")

        # Test proofread
        proof_res = self.client.post(
            "/api/files/proofread",
            json={
                "text": "He go to the laboratory yesterday for doing the experiment.",
    ...
    def test_09_settings_and_profiles(self):
        ...
        self.assertIn("standard_corrector", data["profiles"])
    ...
    def test_10_chat_and_debugger_endpoints(self):
        ...
        chat_res = self.client.post("/api/chat", json={
            "messages": [{"role": "user", "content": "Explain grammar correction in 1 sentence."}],
            "mode": "local_model"
        })
